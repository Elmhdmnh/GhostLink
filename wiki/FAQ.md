...
### Q: `KillAMSI.cpp` 有什么作用？

`KillAMSI.cpp` 是一个独立的 C++ DLL 工具，用于通过注册表禁用 Windows 的 AMSI（Antimalware Scan Interface）脚本扫描功能：

```cpp
// 设置 AmsiEnable = 0
RegSetValueEx(hkey, TEXT("AmsiEnable"), 0, REG_DWORD, ...);
```

**作用**：禁用 AMSI 后，PowerShell、VBScript、VBA 等脚本在运行前不会被实时扫描，可绕过基于 AMSI 的脚本检测。
...
- 重启后生效，或调用后立即生效
- 部分现代 EDR/AV 可能会监控 `AmsiEnable` 注册表变更

### Q: 如何调用 `KillAMSI`？

编译为 DLL 后，可在客户端代码中通过 `ctypes` 加载调用：
